...
function bfgs_qp(g, H; eps_a = 1e-5, eps_r = 1e-5, max_iter = length(g))
    ...
    r0   = norm(r)                       # norme initiale   
    tol  = eps_a + eps_r * r0            # seuil d’arrêt
    q    = 0.0                           # q(x₀) = 0 (car x₀ = 0)
    B    = Matrix{eltype(g)}(I, n, n)    # B₀ = I  (approx. H⁻¹)

    @printf("%4s  %12s  %13s\n", "k", "‖∇q(x)‖", "q(x)")
    @printf("%4d  %12.4e  %13.6e\n", 0, r0, q)

    for k in 1:max_iter
        p  = -B \ r                      # solve  B p = -r   (O(n²))
        Hp = H * p                       # unique produit H p 
        α  = -(r' * p) / (p' * Hp)       # pas exact

        s  = α * p                       # déplacement
        x .+= s
        q += 0.5 * α * (r' * p)          # q_{k+1} = q_k + ½ α rᵀp

        r_new = r + α * Hp               # r_{k+1} = r_k + α H p
        y     = r_new - r                # = α H p
        sy    = dot(s, y)                # sᵀy  (doit être > 0)
